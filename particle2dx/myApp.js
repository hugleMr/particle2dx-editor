    ...
    init: function () {
        ...
        prev_dn.drawPoly(
            [
                cc.p(0, 0),
                cc.p(s_size.width, 0),
                cc.p(s_size.width, s_size.height),
                cc.p(0, s_size.height),
            ],
            cc.c4f(0, 0, 0, 0.8),
            0,
            cc.c4f(0, 0, 0, 1)
    ...
    update: function (dt) {
        ...
        for (ind in emitter) {
            if (emitter[ind].getDuration() != -1) {
                ...
                if (
                    emit_timer[ind] >
                    emitter[ind].getDuration() + emitter[ind].getLife() + 0.5
                ) {
...
function setGrid(interval_px) {
    ...
    for (i = 1; i <= Math.floor(s_size.height / interval_px); i = i + 1) {
        dn.drawSegment(
            cc.p(0, s_size.height - i * interval_px),
            cc.p(s_size.width, s_size.height - i * interval_px),
            gridwidth,
            cc.c4f(0.5, 0.5, 0.5, 0.5)
        );
    }
    //tate
    for (i = 1; i <= Math.floor(s_size.width / interval_px); i = i + 1) {
        dn.drawSegment(
            cc.p(i * interval_px, 0),
            cc.p(i * interval_px, s_size.height),
            gridwidth,
            cc.c4f(0.5, 0.5, 0.5, 0.5)
        );
    }
...
function dumpToInputTag(p_slot) {
    ...
    $("#angle_disp").html(
        emitter[p_slot].getAngle() + "±" + emitter[p_slot].getAngleVar()
    );
    ...
    $("#life_disp").html(
        emitter[p_slot].getLife() + "±" + emitter[p_slot].getLifeVar()
    );
    ...
    $("#start_a_disp").html(
        round2(start_col.a) + "±" + round2(start_col_var.a)
    );
    $("#start_r_disp").html(
        round2(start_col.r) + "±" + round2(start_col_var.r)
    );
    $("#start_g_disp").html(
        round2(start_col.g) + "±" + round2(start_col_var.g)
    );
    $("#start_b_disp").html(
        round2(start_col.b) + "±" + round2(start_col_var.b)
    );

    $("#start_size_pic").attr("width", emitter[p_slot].getStartSize());
    $("#start_size_pic").attr("height", emitter[p_slot].getStartSize());

    $("#start_size_disp").html(
        parseInt(emitter[p_slot].getStartSize()) +
            "±" +
            parseInt(emitter[p_slot].getStartSizeVar())
    );
    ...
    $("#start_spin_disp").html(
        emitter[p_slot].getStartSpin() + "±" + emitter[p_slot].getStartSpinVar()
    );
    ...
    $("#end_size_disp").html(
        parseInt(emitter[p_slot].getEndSize()) +
            "±" +
            parseInt(emitter[p_slot].getEndSizeVar())
    );
    ...
    $("#end_spin_disp").html(
        emitter[p_slot].getEndSpin() + "±" + emitter[p_slot].getEndSpinVar()
    );
    ...
    if (emitter[p_slot].getEmitterMode() == cc.PARTICLE_MODE_GRAVITY) {
        ...
        $("#speedVar_disp").html(
            emitter[p_slot].getSpeed() + "±" + emitter[p_slot].getSpeedVar()
        );
        ...
        $("#rad_accel_disp").html(
            emitter[p_slot].getRadialAccel() +
                "±" +
                emitter[p_slot].getRadialAccelVar()
        );
        ...
        $("#tan_accel_disp").html(
            emitter[p_slot].getTangentialAccel() +
                "±" +
                emitter[p_slot].getTangentialAccelVar()
        );
    } else {
        //radius
        ...
        $("#rotatePerSecondVar_text").val(
            emitter[p_slot].getRotatePerSecondVar()
        );

        $("#max_radius_disp").html(
            emitter[p_slot].getStartRadius() +
                "±" +
                emitter[p_slot].getStartRadiusVar()
        );
        $("#min_radius_disp").html(emitter[p_slot].getEndRadius());
        $("#rotate_par_second_disp").html(
            emitter[p_slot].getRotatePerSecond() +
                "±" +
                emitter[p_slot].getRotatePerSecondVar()
        );
    }
    ...
    if (emitter[p_slot].getEmitterMode() == cc.PARTICLE_MODE_GRAVITY) {
        ...
        drawAngle(
            p_slot,
            emitter[p_slot].getAngle(),
            emitter[p_slot].getSpeed()
        );
        drawGrav(
            p_slot,
            emitter[p_slot].getGravity().x,
            emitter[p_slot].getGravity().y
        );
    ...
    $("#img_check").attr(
        ...
        "data:image/png;base64," +
            base64encode(cc.Codec.GZip.gunzip(base64decode(png_gz_b64[p_slot])))
    );
...
function baseXML2Plist(p_slot, isPNG, p_type, baseStr) {
    ...
    l_xml = l_xml.replace(
        /__startSizeVar__/m,
        emitter[p_slot].getStartSizeVar()
    );
    ...
    l_xml = l_xml.replace(
        /__startSpinVar__/m,
        emitter[p_slot].getStartSpinVar()
    );
    ...
    if (emitter[p_slot].getEmitterMode() == cc.PARTICLE_MODE_GRAVITY) {
        ...
        l_xml = l_xml.replace(
            /__tan_accel__/m,
            emitter[p_slot].getTangentialAccel() * rev
        );
        l_xml = l_xml.replace(
            /__tan_accel_var__/m,
            emitter[p_slot].getTangentialAccelVar()
        ...
        l_xml = l_xml.replace(
            /__rotatePerSecond__/m,
            emitter[p_slot].getRotatePerSecond() * rev
        );
        l_xml = l_xml.replace(
            /__rotatePerSecondVar__/m,
            emitter[p_slot].getRotatePerSecondVar()
        );
    }
    l_xml = l_xml.replace(
        /__maxParticles__/m,
        emitter[p_slot].getTotalParticles()
    );
    ...
    if (isPNG) {
        if (png_gz_b64[p_slot] == "undefined")
            alert("pnz_gz_b64[" + p_slot + "] undefined");
    ...
    l_xml = l_xml.replace(/__.*?__/gm, "0"); //gravity radius のセットしてないほう __****__ として残っている項目を全部0に
...
function refreshSlot() {
    ...
    if (slot_bg == -1 && bg_sprite)
        bg_str = '<b style="color:#EEDDDD;" >BG</b>';
    else if (bg_sprite) bg_str = '<span style="color:#EEDDDD;" >BG</span>';
    else bg_str = "BG";

    this.str +=
        '<a href="javascript:setBGSlot(-1);" id="slot_m1" style="font-size:130%" >' +
        bg_str +
        "</a> ";
    ...
    this.str +=
        '<a href="javascript:setBGSlot(-2);" id="slot_m2" style="font-size:130%" >' +
        fg_str +
        "</a> ";
    ...
    for (ind in emitter) {
        ...
        ind = parseInt(ind);
        this.str +=
            '<a href="javascript:setSlot(' +
            ind +
            '); " id="slot_' +
            ind +
            '" style="font-size:130%" onMouseOver="checkSlot(' +
            ind +
            ');" onMouseOut="checkSlotOff(' +
            ind +
            ')" >Emit' +
            (ind + 1) +
            "</a> ";
    ...
    //offリンク
    this.str +=
        '<a href="javascript:addSlot();" id="plusbutton" style="font-size:130%">(Add Emitter)</a> <br/>';
    for (ind in emitter) {
        ind = parseInt(ind);
        this.str +=
            '<a href="javascript:setSlotOff(' +
            ind +
            ');" id="slot_off_' +
            ind +
            '" style="place-hub:">Off' +
...
function prevSnapshot(index) {
    ...
    if (particle_dict["textureImageData"]) {
        ...
        if (particle_dict["textureImageData"] == "undefined")
            alert("textureImageData undefined");
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            if (particle_dict["textureImageData"]) {
                ...
                if (particle_dict["textureImageData"] == "undefined")
                    alert("textureImageData undefined");
                gzip_png = cc.Codec.Base64.decode(
                    particle_dict["textureImageData"]
                );
                //clog("png_gz_b64:"+p_slot+" "+ particle_dict['textureImageData'].length);
                png_gz_b64_prev = particle_dict["textureImageData"];
            } else {
...
function loadItem(id) {
    ...
    $.ajax({
        ...
        success: function (data) {
            ...
            $("#price_type option[value='" + data.price_type + "']").attr(
                "selected",
                true
            );
        },
            ...
            $.ajax({
                ...
                success: function (data) {
                    ...
                    if (data != "error") {
                        ...
                        layers = cc.Director.getInstance()
                            .getRunningScene()
                            .getChildren();
...
function xml2ary(xml_str) {
    ...
    if (typeof xml_str == "object") {
        // If xml_str is object then use directly
        xmlDoc = xml_str;
    } else {
        // Otherwise parse str
...
function getSnapshot() {
    ...
    $("#snapshots").html(
        $("#snapshots").html() +
            "&nbsp;<a href='" +
            "javascript:xmlStr2emitter(snap[" +
            (snap.length - 1) +
            "],slot)" +
            "' " +
            'onMouseOver="prevSnapshot(' +
            (snap.length - 1) +
            ');" onMouseOut="prevEnd(); "' +
            ">s" +
            (snap.length - 1) +
...
function rotateSlot(p_slot, digree) {
    ...
    emitter[p_slot].setGravity(
        cc.p(
            round2(g.x * Math.cos(rad) - g.y * Math.sin(rad)),
            round2(g.x * Math.sin(rad) + g.y * Math.cos(rad))
        )
    );
...
function changeEmitterSize(p_slot, bairitu) {
    ...
    emitter[p_slot].setSpeedVar(
        Math.floor(emitter[p_slot].getSpeedVar() * bairitu)
    );

    g = emitter[p_slot].getGravity();
    emitter[p_slot].setGravity(
        cc.p(Math.floor(g.x * bairitu), Math.floor(g.y * bairitu))
    );

    pv = emitter[p_slot].getPosVar();
    emitter[p_slot].setPosVar(
        cc.p(Math.floor(pv.x * bairitu), Math.floor(pv.y * bairitu))
    );

    emitter[p_slot].setStartSize(
        Math.floor(emitter[p_slot].getStartSize() * bairitu)
    );
    emitter[p_slot].setEndSize(
        Math.floor(emitter[p_slot].getEndSize() * bairitu)
    );
...
function size4Slot(base_num) {
    ...
    for (i = 1; i <= 4; i++) {
        ...
        bairitu = 1 + i * 0.4;
        emitter[ind2].setPosition(
            emitter[base_num].getPositionX(),
            emitter[base_num].getPositionY() + i * 140
        );
...
function speed4Slot(base_num) {
    ...
    for (i = 1; i <= 6; i++) {
        ...
        emitter[ind].setSpeedVar(
            emitter[base_num].getSpeedVar() * (1 - i * 0.1)
        );
    }
}
...
function keyShortCut(keycode) {
    ...
    // 38 40 up down
    if (keycode == 38) {
        changeEmitterSize(slot, 1.2);
    }
    if (keycode == 40) {
        changeEmitterSize(slot, 0.8);
    }
    ...
    if (keycode == 71) {
        toggleGravityRadius("gravity", slot);
    }
    if (keycode == 82) {
        toggleGravityRadius("radius", slot);
    }
    ...
    if (keycode == 66) {
        toggleTopleftPane("background");
    }
    if (keycode == 84) {
        toggleTopleftPane("template");
    }
    if (keycode == 73) {
        toggleTopleftPane("import");
    }
    // changePane  t:84 template m:77 motion
    if (keycode == 77) {
        toggleTopleftPane("motion");
    }
    if (keycode == 67) {
        toggleTopleftPane("shape");
    } //  c:67 color pane   s:83 shapes
...
function togglePane(pane_name) {
    $("#pane_" + pane_name).slideToggle(100);
}
...
$(window).keydown(function (e) {
    clog(
        "keydown:" +
            ...
            " alt:" +
            e.altKey
    ); //  KEYCODE  Com:91 Shift:16  Esc:27  Delete:8  Tab:9 Enter:13 Option:18  Spc:32
...
function setGuideCircle(p_slot) {
    ...
    //開始範囲
    guiderect[p_slot].drawDot(
        cc.p(0, 0),
        emitter[p_slot].getStartRadius() + emitter[p_slot].getStartRadiusVar(),
        cc.c4f(0.5, 0.5, 0.5, 0.3)
    ); //centerPos hankei color4F
    guiderect[p_slot].drawDot(
        cc.p(0, 0),
        emitter[p_slot].getStartRadius() - emitter[p_slot].getStartRadiusVar(),
        cc.c4f(0.5, 0.5, 0.5, 0.3)
    ...
    ppp = cc.p(
        c_size.width / 2 + (Math.cos(rad_angle) - Math.sin(rad_angle)) * hankei,
...
function saveFile(text, name, extention) {
